...
fk,dGammakCorr,dGammakCorr_nsn,PGG,PS = np.loadtxt('./LyASolvedGammaCorrAAL_W14B21_bG3_tG100_tQ1_z6.0.out',unpack = True)
...
def LyALAEPkl(zred_out,b_LAE=3.0,b_delta=0.707,
              b_Gamma=-0.173,tau_eff=6, lorder=0,debug=False):
    '''
    ...
    if debug: print("taus {:.4f}".format((b_LAE - tau_eff*b_delta)* \
                    (tau_eff*b_Gamma)))
    PkLAEG = -2.0 * np.ones(lenk)*((b_LAE - tau_eff*b_delta)* \
    ...
    Pk_LAE_nsn = PkLAEl + PkLAEGl + np.ones(lenk)*(b_Gamma*b_Gamma) * \
                 dGammakCorr_nsn*(tau_eff*tau_eff)
    Pk_LAE = PkLAEl + PkLAEGl + np.ones(lenk)*(b_Gamma *b_Gamma) * \
                 dGammakCorr*(tau_eff*tau_eff)

    return Pk,PkLAEl,PkLAEGl,Pk_LAE,Pk_LAE_nsn


def LyALAEXil_fft(zred_out,lorder=0,b_LAE=3.0,b_delta=0.707,
              b_Gamma=-0.173,tau_eff=6,debug=False):
    '''
    ...
    Pk_z,PkLAEl_z,PkLAEGl_z,Pk_LAE_z,Pk_LAE_nsn_z = LyALAEPkl(zred_out,b_LAE=b_LAE,
                        b_delta=b_delta,b_Gamma=b_Gamma,tau_eff=tau_eff,lorder = lorder)
...
def LyALAEXiangComps(sigr,zred_out,b_LAE=3.0,b_delta=0.707,
    ...
    r,r2xi0LL,r2xi0LG,r2xi0GG,r2xi0GG_nsn = LyALAEXil_fft(zred_out,0,b_LAE=b_LAE,
                        b_delta=b_delta,b_Gamma=b_Gamma,tau_eff=tau_eff,debug=debug)

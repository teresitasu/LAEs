...
fk,PGG,dGammakCorr_nsn,dGammakCorr,PS = np.loadtxt('./LyASolvedGammaCorrAAL_W14B21_bG3_tG100_tQ1_z6.0.out',unpack = True)
...
def LyATransXilInt_fft(fk,fkk,Pk_alpha_z,lorder=0):
    ...
    for j in np.arange(np.int(n/2)+2,n):
        if lorder == 0:
            Yk[0][j] = -Yk[0][n+2-j]
...
def LyALAEPkl(zred_out,b_LAE=3.0,b_delta=0.707,
              b_Gamma=-0.17,tau_eff=6, lorder=0):
    ...
    PkLAE = np.ones(lenk) * (b_LAE - tau_eff*b_delta) * deltk
    PkLAE = PkLAE * PkLAE
    PkLAEG = -2.0 * np.ones(lenk)*((b_LAE - tau_eff*b_delta)*(tau_eff*b_Gamma)) * ddGammak_nsn
    ...
    Pk_LAE_nsn = PkLAEl + PkLAEGl + np.ones(lenk)*(b_Gamma*b_Gamma) * dGammakCorr_nsn*(tau_eff*tau_eff)
    Pk_LAE = PkLAEl + PkLAEGl + np.ones(lenk)*(b_Gamma *b_Gamma) * dGammakCorr*(tau_eff*tau_eff)

    #print('PKLAE',PkLAEl_z[0],PkLAEGl_z[0],Pk_LAE_z[0],Pk_LAE_nsn_z[0])
    return Pk,PkLAEl,PkLAEGl,Pk_LAE,Pk_LAE_nsn


def LyALAEXil_fft(zred_out,lorder=0):
    '''
    ...
    Pk_z,PkLAEl_z,PkLAEGl_z,Pk_LAE_z,Pk_LAE_nsn_z = LyALAEPkl(zred_out,lorder = lorder)
    ...
    #print("lenr {:10.3e} {:10.3e} {:10.3e} {:10.3e} ".format(PK_LL_z[0]
    #                    ,PK_LG_z[0],PK_GG_z[0],PK_GG_nsn_z[0]))
    #print('Xil_fft',PkLAEl_z[0],PkLAEGl_z[0],Pk_LAE_z[0],Pk_LAE_nsn_z[0])
    ...
    aa = 40
    #print("Ykll {:10.3e} {:10.3e} {:10.3e} {:10.3e} ".format(YkLL[0][aa]
    #                    ,YkLG[0][aa],YkLG[0][aa],YkGG_nsn[0][aa]))
    if lorder == 0:
        Yk1 = YkLL[0]
        ...
        aa = 400
        #print("xill {:10.3e} {:10.3e} {:10.3e} {:10.3e} ".format(xilLL[aa]
...
def LyALAEXiangComps(sigr,zred_out):
    '''
    ...
    r,r2xi0LL,r2xi0LG,r2xi0GG,r2xi0GG_nsn = LyALAEXil_fft(zred_out,0)
    r,r2xi2LL,r2xi2LG,r2xi2GG,r2xi2GG_nsn = LyALAEXil_fft(zred_out,2)
    r,r2xi4LL,r2xi4LG,r2xi4GG,r2xi4GG_nsn = LyALAEXil_fft(zred_out,4)
    ...
    return theta, xiaLL, xiaLG, xiaGG_nsn, xiaGG   
